...
void coluna(int** matriz_a, int** matriz_b, int** matriz_c) {
  for (int j = 0; j < X; j++) { // Percorre as colunas da matriz resultado
      for (int i = 0; i < M; i++) { // Percorre as linhas da matriz resultado
          matriz_c[i][j] = 0; // Inicializa a posição da matriz resultado

          for (int k = 0; k < N; k++) { // Percorre os elementos para multiplicação
...
int main(){
  ...
  coluna(matriz_a, matriz_b, matriz_c);
  ...
  std::cout << "\nTempo de execução percorrendo por coluna: " << tempo_decorrido << " ms" << std::endl;
}